...
- `src/cli/antigravity`: Wraps `agy` CLI execution, parses output events, and discovers available models.
- `src/session`: Manages session keys, state serialization, and timezone daily resets.
- `src/telegram`: Telegram bot event handler, message reply construction, and Markdown-to-HTML parser.
- `src/background`: PTY executor wrapping spawned CLI processes with safe async cancellation.
