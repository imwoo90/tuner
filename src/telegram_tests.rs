...
mod tests {
    ...
    use crate::telegram::TEST_ENV_MUTEX;
    ...
    async fn test_telegram_session_manager_integration() {
        ...
        let (mgr, cfg, cli, bot, cron_mgr, topic_cache, bot_info, mgm, _env) = setup_e2e_env(&temp_dir, "#!/bin/sh\necho '{\"result\":\"ok\",\"is_error\":false}'\n");
        ...
        assert!(handle_message(bot, msg, cfg, mgr, cli, cron_mgr, topic_cache, bot_info, mgm).await.is_ok());
    }
    ...
    async fn test_normal_message_flow_e2e() {
        ...
        let res1 = handle_message(bot.clone(), msg, cfg.clone(), mgr.clone(), cli.clone(), cron_mgr.clone(), topic_cache.clone(), bot_info.clone(), mgm.clone()).await;
        ...
        let res2 = handle_message(bot, follow_up, cfg, mgr.clone(), cli, cron_mgr, topic_cache, bot_info, mgm).await;
    ...
    async fn test_commands_routing_e2e() {
        ...
        for cmd in &["/status", "/memory", "/stop", "/model", "/cron"] {
            ...
            assert!(handle_message(bot.clone(), msg, cfg.clone(), mgr.clone(), cli.clone(), cron_mgr.clone(), topic_cache.clone(), bot_info.clone(), mgm.clone()).await.is_ok());
        }
    }
    ...
    async fn test_directive_parsing_e2e() {
        ...
        let _ = handle_message(bot.clone(), msg1, cfg.clone(), mgr.clone(), cli.clone(), cron_mgr.clone(), topic_cache.clone(), bot_info.clone(), mgm.clone()).await;
